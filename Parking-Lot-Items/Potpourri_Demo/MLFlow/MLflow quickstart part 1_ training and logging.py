...
def train_diabetes(data, in_alpha, in_l1_ratio, client, data_size):
  ...
  with mlflow.start_run():
    ...
    mlflow.set_tags({"client_name": client, "data_size": data_size})
...
train_diabetes(data, 0.01, 0.01, 'client1', 10000)
...
train_diabetes(data, 0.01, 0.75, 'client2', 10000000)
...
train_diabetes(data, 0.01, .5, 'client1', 10000000)
...
train_diabetes(data, 0.01, 1, 'client3', 10000000)
...
# DBTITLE 1,Load data from the notebook experiment
...
# DBTITLE 1,Load data using experiment IDs
df = spark.read.format("mlflow-experiment").load("893693594044182")
display(df)

# COMMAND ----------

# DBTITLE 1,Load data using experiment name
...
# DBTITLE 1,Analyze MLflow runs programmatically
filtered_df = df.filter("tags.client_name == 'client1' AND params.alpha > '0.001'")
display(filtered_df)
